...
struct tmcuart_s {
    struct timer timer;
    UARTPinTypeDef tx_pin;
    UARTPinTypeDef rx_pin;
    uint8_t flags;
...
static void tmcuart_reset_line(struct tmcuart_s *t) {
    if (t->flags & TU_SINGLE_WIRE)
        ...
        HAL_GPIO_WritePin(t->tx_pin.port, t->tx_pin.pin, GPIO_PIN_RESET);
    else
        //gpio_out_write(t->tx_pin, 1);
        HAL_GPIO_WritePin(t->tx_pin.port, t->tx_pin.pin, GPIO_PIN_SET);
...
void command_config_tmcuart(uint32_t *args) {
...
void command_tmcuart_send(uint32_t *args) {
