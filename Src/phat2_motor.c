...
void phat2_motors_init() {
    ...
    tmc2209_init(&TMC2209_S1, 0, 1, TMC2209_S1_cfg, tmc2209_defaultRegisterResetState);
...
void tmc2209_readWriteArray(uint8_t channel, uint8_t *data, size_t writeLength, size_t readLength) {
    ...
    // TODO: add selection of uart based on ....channel? which 
    // seems to be put in place by TRINAMIC for this
    //HAL_GPIO_TogglePin(H1_GPIO_Port,H1_Pin);

    // read write using HAL
    HAL_UART_Transmit_DMA(&huart2, data, writeLength);
    //HAL_UART_Transmit(&huart2, data, writeLength, 10);

    if(readLength>0)
        //HAL_UART_Receive(&huart2, data, readLength, 10);
